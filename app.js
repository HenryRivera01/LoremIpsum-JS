...
form.addEventListener('submit', (e)=>{
    e.preventDefault();
    clearDisplay()
    const paragraphCount = parseInt(input.value) ; //All inputs returns a string
    //if u generate without type any number the form gets NaN value
    displayLorem(paragraphCount)
})
...
function displayLorem(amount){
    if(!isValidAmount(amount)){    
        resultSection.innerHTML += 
        `<p>${loremParagraphs[getRandomNumber()]}</p>`
    }else{
        for (let i = 0; i < amount; i++) {
             resultSection.innerHTML += 
        `<p>${loremParagraphs[i]}</p>`
        }
    }

}

function isValidAmount(paragraphCount){
    return !isNaN(paragraphCount) && paragraphCount >= MIN_PARAGRAPHS && paragraphCount <= MAX_PARAGRAPHS
}

function getRandomNumber(){
    return randomIndex = Math.floor(Math.random() * loremParagraphs.length) 
}
